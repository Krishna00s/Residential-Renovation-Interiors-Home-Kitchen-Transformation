  ...
  return (
    <div className="mx-auto max-w-7xl px-6 md:px-12 py-16 space-y-12 bg-[#161D18] text-[#EDE8DF]">
      ...
      <div className="space-y-4">
        ...
        <h1 className="font-serif text-4xl md:text-6xl lg:text-7xl font-normal text-[#EDE8DF]">
          {project.title}
        </h1>
      ...
      {/* Statistics Bar */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-8 rounded-2xl bg-[#1B231D] border border-[#EDE8DF]/15">
        {project.stats.map((st, idx) => (
          <div key={idx} className="space-y-1">
            <span className="font-sans text-xs text-[#8E877D] uppercase tracking-wider">{st.label}</span>
            <p className="font-serif text-2xl font-semibold text-[#EDE8DF]">{st.value}</p>
          </div>
        ))}
      </div>

      {/* Next Project Link */}
      <div className="pt-12 border-t border-[#EDE8DF]/15">
        <Link
          to={`/projects/${nextProject.slug}`}
          className="group block p-8 rounded-2xl bg-[#1B231D] text-[#EDE8DF] transition-all hover:bg-[#222B24] border border-[#EDE8DF]/10"
          data-cursor="hover"
        ...
        >
          ...
          <h2 className="mt-4 font-serif text-3xl font-normal text-[#EDE8DF]">
            {nextProject.title}
          </h2>
          <p className="mt-1 font-sans text-xs text-[#8E877D]">{nextProject.category} • {nextProject.location}</p>
        </Link>
      </div>
    </div>
  );
