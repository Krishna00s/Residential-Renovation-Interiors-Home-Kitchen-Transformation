...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <Link
      ...
      className={`group relative block w-full overflow-hidden rounded-2xl bg-[#1C241E] text-[#F1EDE4] transition-all duration-500 hover:shadow-2xl ${heightClass} ${className}`}
      data-cursor="hover"
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#1C241E] via-[#1C241E]/40 to-transparent transition-opacity duration-500 group-hover:opacity-80" />

      {/* Asymmetrical Floating Header Badge */}
      <div className="absolute top-6 left-6 right-6 z-10 flex items-center justify-between pointer-events-none">
        <span className="rounded-full bg-[#1C241E]/80 backdrop-blur-md px-3.5 py-1.5 font-sans text-[10px] font-bold tracking-[0.2em] text-[#C5A880] uppercase border border-[#F1EDE4]/10">
          {project.category}
        </span>
        <span className="font-sans text-xs text-[#F1EDE4]/70 uppercase tracking-widest font-medium">
          {project.year}
        </span>
      </div>
      ...
      <div className="absolute inset-0 z-10 p-8 md:p-10 flex flex-col justify-end">
        <div className="space-y-3 transform transition-transform duration-500 group-hover:-translate-y-1">
          <div className="flex items-center space-x-2 font-sans text-xs text-[#7E776D] uppercase tracking-wider">
            <span>{project.location}</span>
            <span>•</span>
            <span>{project.duration}</span>
          </div>

          <h3 className="font-serif text-3xl md:text-4xl font-normal leading-tight text-[#F1EDE4] group-hover:text-[#C5A880] transition-colors">
            {project.title}
          </h3>

          <p className="font-sans text-xs md:text-sm text-[#F1EDE4]/80 line-clamp-2 leading-relaxed max-w-lg">
            {project.subtitle}
          </p>
